...
def var_historical(returns: pd.Series, alpha: float = 0.95) -> float:
    ...
    -------
    float
        Positive magnitude of the (1 - alpha) lower-tail loss.
        If the lower-tail quantile is >= 0 (all non-negative returns),
        VaR is 0.0.
    """
    r = pd.Series(returns).dropna()

    # Use a discrete lower-tail quantile to match common historical VaR conventions
    # and avoid interpolation pulling the quantile toward 0.
    q = 1.0 - alpha
    try:
        # numpy >= 1.22
        tail = np.nanquantile(r, q, method="lower")
    except TypeError:
        # numpy < 1.22 fallback
        tail = np.nanquantile(r, q, interpolation="lower")

    # Convert to positive loss magnitude; no negative VaR
