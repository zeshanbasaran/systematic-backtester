...
start = st.sidebar.date_input("Start Date", pd.to_datetime(START))
end = st.sidebar.date_input("End Date", pd.to_datetime(END))
...
with tab2:
    ...
    st.write("Breaches detected:" if breaches else "No breaches detected.")

    if breaches:
        st.error(breaches)

    st.metric("VaR 95%", f"{stats['var95']:.2%}")
    st.metric("Annual Vol", f"{stats['vol']:.2%}")
