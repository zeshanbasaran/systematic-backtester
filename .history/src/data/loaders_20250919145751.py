...
def _standardize_columns(df: pd.DataFrame, symbol: str | None = None) -> pd.DataFrame:
    """Rename Yahoo columns to ['open','high','low','close','adj_close','volume'],
    handle MultiIndex (ticker-prefixed) cases, ensure tz-naive DatetimeIndex."""
    cols = ["open", "high", "low", "close", "adj_close", "volume"]
    if df is None or df.empty:
        return pd.DataFrame(columns=cols).astype({c: "float64" for c in cols})

    # If MultiIndex like (TICKER, Field) or (Level0, Level1), prefer taking the Field slice.
    if isinstance(df.columns, pd.MultiIndex):
        # Try to drop a ticker level if present
        # Case 1: top level has the symbol
        if symbol is not None:
            up = symbol.upper()
            top_vals = [str(v).upper() for v in df.columns.get_level_values(0)]
            if up in top_vals:
                df = df.xs(symbol, axis=1, level=0, drop_level=True)
        # If still MultiIndex (rare), take the last level (= the actual OHLCV names)
        if isinstance(df.columns, pd.MultiIndex):
    ...
    # Ensure DatetimeIndex (tz-naive), sorted, deduped
    if not isinstance(df.index, pd.DatetimeIndex):
        df.index = pd.to_datetime(df.index, errors="coerce")
    if getattr(df.index, "tz", None) is not None:
        # drop timezone info but keep wall clock times
...
def _fetch_from_yahoo(symbol: str, start: str, end: str, bar: str) -> pd.DataFrame:
    ...
    # Clamp hourly to ~2 years to avoid empty frames from Yahoo
    if bar == "1h":
    ...
    last_err = None
    for threads in (False, True):  # some Windows/Python combos behave better with threads=False
        try:
            df = yf.download(
                symbol,
                start=start,
            ...
            out = _standardize_columns(df, symbol)
            if not out.empty:
                return out
    ...
    # Return empty shaped frame if nothing worked
