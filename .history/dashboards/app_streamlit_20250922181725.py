...
with tab2:
    ...
    # headline cards for the three core risk metrics (always shown)
    c1, c2, c3 = st.columns(3)
    c1.metric("Max Drawdown", pct(stats["dd"]), delta=f"limit −{thr.max_dd*100:.0f}%")
    c2.metric("VaR 95% (1-day)", pct(stats["var95"]), delta=f"limit {thr.var_95*100:.0f}%")
    c3.metric("Annual Vol", pct(stats["vol"]), delta=f"limit {thr.vol_ann*100:.0f}%")
